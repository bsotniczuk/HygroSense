...
public class SampleUtil {
    ...
    public static KeyStorePasswordPair getKeyStorePasswordPair(MainActivity mainActivity, final String certificateFile, final String privateKeyFile) {
        return getKeyStorePasswordPair(mainActivity, certificateFile, privateKeyFile, null);
    }
    ...
    public static KeyStorePasswordPair getKeyStorePasswordPair(MainActivity mainActivity, final String certificateFile, final String privateKeyFile,
        ...
        final PrivateKey privateKey = loadPrivateKeyFromFile(mainActivity, privateKeyFile, keyAlgorithm);
    ...
    private static List<Certificate> loadCertificatesFromFile(MainActivity mainActivity, final String filename) {
        InputStream is;
        try {
            is = mainActivity.getAssets().open(filename);
            final CertificateFactory certFactory = CertificateFactory.getInstance("X.509");
            return (List<Certificate>) certFactory.generateCertificates(is);
    ...
    private static PrivateKey loadPrivateKeyFromFile(MainActivity mainActivity, final String filename, final String algorithm) {
        PrivateKey privateKey = null;
        InputStream is;
        try {
            is = mainActivity.getAssets().open(filename);
            privateKey = PrivateKeyReader.getPrivateKey(is, algorithm);
